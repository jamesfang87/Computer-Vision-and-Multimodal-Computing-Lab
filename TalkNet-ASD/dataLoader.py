...
def generate_audio_set(dataPath, batchList) -> dict:
    audioSet = {}
    for line in batchList:
        data = line.split('\t')
        videoName = data[0][:13] # 13 for AVDIAR
        dataName = data[0]
...
def load_audio(data, numFrames: int, audioAug: bool, audioSet: dict):
    dataName = data[0]
    fps = float(data[2])
...
class TrainLoader():
    def __init__(self, trialFileName, audioPath, visualPath, batchSize, **kwargs):
        ...
        sortedMixList = sorted(mixLst, key=lambda data: (int(data.split('\t')[1]), int(data.split('\t')[-1])), reverse=True)         
        start = 0        
        while True:
          length = int(sortedMixList[start].split('\t')[1])
          end = min(len(sortedMixList), start + max(int(batchSize / length), 1))
          self.miniBatch.append(sortedMixList[start: end])
          if end == len(sortedMixList):
              break
          start = end     
    ...
    def __getitem__(self, index):
        ...
        for line in batchList:
            data = line.split('\t')            
            audioFeatures.append(load_audio(data, numFrames, audioAug=True, audioSet=audioSet))
...
class ValLoader():
    ...
    def __getitem__(self, index):
        ...
        data = line[0].split('\t')
        audioFeatures = [load_audio(data, numFrames, audioAug=False, audioSet=audioSet)]
